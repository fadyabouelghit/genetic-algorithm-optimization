function [user_positions, df_users_table, total_connected_users, total_transmitted_pwr] = SINREvaluation(antenna_object, power_status, tx_x, tx_y, tx_height, no_fbs, tx_power, mbs_x, mbs_y, mbs_height, mbs_power, subset_x_min, subset_x_max, subset_y_min, subset_y_max, num_users, threshold, containsMbs, antennaObjectMbs)
...
%     df_users_table : Structured table of power and SINR values
    ...
    for fbs_id = 1:no_fbs
        if power_status(fbs_id)
            P = calculate_power(antenna_object, tx_x(fbs_id), tx_y(fbs_id), tx_height(fbs_id), ...
                                tx_power(fbs_id), subset_x_min, subset_x_max, subset_y_min, subset_y_max);
        else
            P = zeros(subset_x_max + 1, subset_y_max + 1);
        end
        idx = sub2ind(size(P), user_positions(:, 1), user_positions(:, 2));
